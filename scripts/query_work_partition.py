...
def partition(plan: dict[str, Any], contract: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    """Route only an exact, typed, one-metric rate to the existing compiler."""
    approved = contract.get("approved")
    questions = plan.get("questions")
    require(isinstance(approved, list) and isinstance(questions, list), "approved metrics or plan questions are unavailable")
